...
require "inspec-tfkit/profile_helper"
require "inspec-tfkit/terraform/generate"
require "inspec-tfkit/terraform/negative"

module InspecPlugins::Tfkit
  module Terraform
    class CliCommand < Inspec.plugin(2, :cli_command)
      ...
      def generate
        ...
        generated_controls = InspecPlugins::Tfkit::Terraform::Generate.parse_generate(options[:tfstate], resource_path, platform)
        printable_controls = InspecPlugins::Tfkit::InspecHelper.tf_controls(options[:title], generated_controls, platform)
        InspecPlugins::Tfkit::ProfileHelper.render_profile(ui, options, options[:tfstate], printable_controls, platform)
        exit 0
      end
      ...
      def negative
        ...
        negative_controls = InspecPlugins::Tfkit::Terraform::Negative.parse_negative(options[:tfstate], resource_path, platform)
        printable_controls = InspecPlugins::Tfkit::InspecHelper.tf_controls(options[:title], negative_controls, platform)
        InspecPlugins::Tfkit::ProfileHelper.render_profile(ui, options, options[:tfstate], printable_controls, platform)
        exit 0
      end
    end
