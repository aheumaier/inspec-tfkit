...
module InspecPlugins::Tfkit::Terraform
  class Negative
    ...
    def self.parse_negative(tf_file, resource_path, platform)
      ...
      parsed_resources = InspecPlugins::Tfkit::Terraform::Generate.parse_resources(tfstate, resource_path, platform)
    ...
    def self.parse_unmatched_resources(resources, sourcefile, platform)
      ...
      unmatched_resources = InspecPlugins::Tfkit::InspecHelper.available_resource_iterators(platform).keys - resources.keys
      ...
      unmatched_resources.each do |unmatched|
        unresources = InspecPlugins::Tfkit::InspecHelper.available_resource_iterators(platform)[unmatched]
        iterator = unresources["iterator"]
        ctrl = Inspec::Control.new
        ctrl.id = "NEGATIVE-COVERAGE:#{iterator}"
        ctrl.title = "inspec-tfkit NEGATIVE-COVERAGE:#{iterator}"
    ...
    def self.parse_matched_resources(resources, sourcefile, platform) # rubocop:disable Metrics/AbcSize
      ...
      resources.keys.each do |resource|
        ...
        resource_iterators = InspecPlugins::Tfkit::InspecHelper.available_resource_iterators(platform)[resource]
        ...
        ctrl += "    Generated by inspec-tfkit v#{InspecPlugins::Tfkit::VERSION}\"\n\n"
        ctrl += "  impact 1.0\n"
        ...
        first = true
        InspecPlugins::Tfkit::InspecHelper.available_resource_qualifiers(platform)[resource].each do |parameter|
          ...
          properties = InspecPlugins::Tfkit::InspecHelper.available_translated_resource_properties(platform, resource)
          ...
          Inspec::Log.debug "tfkit::Terraform::Negative.parse_matched_resources #{resource} qualifier found = #{property} MATCHED"
