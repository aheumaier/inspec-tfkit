  ...
  module Tfkit
    class InspecHelper
      ...
      TRANSLATED_RESOURCES = {
        ...
        "azurerm_resource_group" => "azure_resource_groups",
        "azurerm_virtual_machine" => "azure_virtual_machine",
        "azurerm_linux_virtual_machine" => "azure_virtual_machine"
      ...
      def self.available_resource_qualifiers(platform)
        case platform
        when "aws"
          InspecPlugins::Tfkit::Platforms::AwsHelper::AWS_RESOURCE_QUALIFIERS
        when "azure"
          InspecPlugins::Tfkit::Platforms::AzureHelper::AZURE_RESOURCE_QUALIFIERS
        when "gcp"
          InspecPlugins::Tfkit::Platforms::GcpHelper::GCP_RESOURCE_QUALIFIERS
        end
      end
      ...
      def self.available_resource_iterators(platform)
        case platform
        when "aws"
          InspecPlugins::Tfkit::Platforms::AwsHelper::AWS_RESOURCE_ITERATORS
        when "azure"
          InspecPlugins::Tfkit::Platforms::AzureHelper::AZURE_RESOURCE_ITERATORS
        when "gcp"
          InspecPlugins::Tfkit::Platforms::GcpHelper::GCP_RESOURCE_ITERATORS
        end
      end
      ...
      def self.available_translated_resource_properties(platform, resource)
        case platform
        when "aws"
          InspecPlugins::Tfkit::Platforms::AwsHelper::AWS_TRANSLATED_RESOURCE_PROPERTIES[resource]
        when "azure"
          InspecPlugins::Tfkit::Platforms::AzureHelper::AZURE_TRANSLATED_RESOURCE_PROPERTIES[resource]
        when "gcp"
          InspecPlugins::Tfkit::Platforms::GcpHelper::GCP_TRANSLATED_RESOURCE_PROPERTIES[resource]
        end
      end
      ...
      def self.resource_properties(resource, platform)
        ...
        case platform
        when "aws"
          inspec_properties -= InspecPlugins::Tfkit::Platforms::AwsHelper::AWS_REMOVED_PROPERTIES[resource] unless InspecPlugins::Tfkit::Platforms::AwsHelper::AWS_REMOVED_PROPERTIES[resource].nil?
        when "azure"
          inspec_properties -= InspecPlugins::Tfkit::Platforms::AzureHelper::AZURE_REMOVED_PROPERTIES[resource] unless InspecPlugins::Tfkit::Platforms::AzureHelper::AZURE_REMOVED_PROPERTIES[resource].nil?
        when "gcp"
          inspec_properties -= InspecPlugins::Tfkit::Platforms::GcpHelper::GCP_REMOVED_PROPERTIES[resource] unless InspecPlugins::Tfkit::Platforms::GcpHelper::GCP_REMOVED_PROPERTIES[resource].nil?
        end
        # get InSpec properties by method names
        inspec_properties.collect!(&:to_s)
        # Inspec::Log.debug "InspecHelper.resource_properties #{resource} properties = #{JSON.pretty_generate(inspec_properties)}"

        inspec_properties
      end

      def self.tf_controls(title, generated_controls, platform)
        Inspec::Log.debug "Calling InspecPlugin::InspecHelper::tf_controls"
        content = "title \"#{title}: generated by tfkit v#{Tfkit::VERSION}\"\n"

        content += InspecPlugins::Tfkit::Platforms::AwsHelper.tf_controls if platform.eql?("aws")
        content += InspecPlugins::Tfkit::Platforms::AzureHelper.tf_controls if platform.eql?("azure")

        # write all controls
        generated_controls.flatten.each do |control|

          # JSON.pretty_generate(control.inspect)
